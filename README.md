...
- **Real-Time Booking & Order Tracking:** Customer lookup dashboard tracking order status transitions (`Initial`, `Done`, `Cancelled`) via unique booking codes.
...
- **Order & Customer Fulfillment:** Searchable order management inbox with direct status updates (`Initial`, `Done`, `Cancelled`), contact inquiry messages, and customer email dispatch via Nodemailer.
...
│   ├── admin/                 # Back-office CSS styles, JavaScript scripts, & icons
